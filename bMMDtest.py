...
"""
This code corresponds to the method presented in Zaremba et al. (2013).
"""
...
def bMMDtest(
    ...
    stat_total = vmap(lambda x,y: bMMDstat(x, y, bandwidth, kernel))(X_total,Y_total) # (B+1)
...
def bMMDstat(X, Y, bandwidth=1, kernel='gaussian'):
    m, d = X.shape
    n = Y.shape[0]
    n = min(n, m)
    ...
    X_reshape=jnp.reshape(X,((num_blocks).astype(int) ,b.astype(int),d))
    Y_reshape=jnp.reshape(Y,((num_blocks).astype(int) ,b.astype(int),d))
